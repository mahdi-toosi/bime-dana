                ...
                <div class="card">
                    <div class="boxtitle">
                            <span class="d-flex align-items-center">
                                <i class="la la-plus"></i>ویرایش مدل خودرو  <span class="text-success mr-2">{{ $carModel->title }}</span>
                            </span>
                    ...
                    <div class="container-fluid mt-2 mb-3">
                        <div class="row justify-content-center">
                            <div class="col-lg-6 col-md-8">
                                <form action="{{ route('admin.car-models.update', ['car_model' => $carModel->id]) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <div class="row mb-5">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input class="form-control " name="title" value="{{ $carModel->title }}" id="title" required>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="custom-select" id="carType" name="carType" required onchange="getPlans(this)">
                                                    ...
                                                    @foreach($carTypes as $car)
                                                        <option value="{{$car->id}}" @if($carModel->car_type_id == $car->id) selected @endif>{{$car->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="custom-select" id="plans" name="plans" required>
                                                    ...
                                                    @foreach($plans as $plan)
                                                        <option value="{{$plan->id}}" @if($carModel->plan_id == $plan->id) selected @endif>{{$plan->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <button name="add"
                                                    class="btn btn-info d-flex align-items-center justify-content-center mr-auto ml-0 mt-2"
                                                    id="add">
                                                ویرایش مدل خودرو
                                            </button>
